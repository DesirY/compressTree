...
Link.prototype.getLinkPath = function (){
  ...
  let hei = nodeHei+gap;  // 增加节点的宽度,使节点与边之间留出空隙
  let pathAttr;
  if (this.plot.length > 0){
    ...
    pathAttr = "M" + [srcStartX + band*(this.target.broID+1), this.source.y+hei/2].join(" ")
        + " L" + [srcStartX + band*(this.target.broID), this.source.y+hei/2].join(" ")
        + "L" +[this.target.x - (this.target.extension>1? 1:this.target.extension)*nodeWid/2, this.target.y-hei/2].join(" ")
        + " L" + [this.target.x-tgtWid/2, this.target.y-hei/2].join(" ")
        + " L" + [this.target.x+tgtWid/2, this.target.y-hei/2].join(" ")
        + " L" + [this.target.x + (this.target.extension>1? 1:this.target.extension)*nodeWid/2, this.target.y-hei/2]+" Z";
  }
